...
def train_result():
    ...
    for _, img in pos_64x128_dir:
        conf_list.append(detector.run_on_window_level(img))
        label_list.append(1)
...
def eval_64x128():
    conf_list, label_list = [], []
    for f, img, label in test_64x128_dir:
        conf_list.append(detector.run_on_window_level(img))
...
def eval_full(thr_conf, thr_iou, onlyPos=True):    
    ...
    for f, img, label in tqdm(test_dir):
        ...
        bbox, conf = detector(img, stride, pyr_step)
...
if __name__ == '__main__':    
    ...
    detector = joblib.load(str(model_64x128_dir))
