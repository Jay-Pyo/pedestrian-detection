...
def report_accuracy(confidences, label_vector):
  ...
  tp = np.logical_and(preds == 1, preds==label_vector)
  fp = np.logical_and(preds == 1, preds!=label_vector)
  ...
  print_line = 'Accuracy = {:4.3f}%\nTrue Positive rate = {:4.3f}%\nFalse Positive rate = {:4.3f}%\nTrue Negative rate = {:4.3f}%\nFalse Negative rate = {:4.3f}%'.format(accuracy, tp_rate, fp_rate, tn_rate, fn_rate)
...
def visualize_detections_by_image(bboxes, confidences, image_ids, tp, fp,
  ...
  for gt_file in gt_file_list:
    ...
    plt.savefig(osp.join(save_dir, 'fig-{0}.png'.format(gt_file[:-4])))
